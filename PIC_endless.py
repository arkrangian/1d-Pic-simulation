...
class CumulativeInterpolater:
    ...
    def linearInterpolate(location_array:np.ndarray, grid_length, grid_number) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        count = np.zeros(grid_number)
        sRDC = np.zeros(grid_number)      # Sum of Relative Distance in Cell
        ...
        result[1:] = sRDC[0:-1] - sRDC[1:] + grid_length * count[1:]
        result[0] = (count[0] * grid_length - sRDC[0]) + sRDC[-1]
...
class PoissonSolver:
    def __init__(self, gridNum, gridLength):
        ...
        temp = np.zeros((gridNum, gridNum))
        for i in range(gridNum-1):
            temp[i][i] = 2
            temp[i][i+1] = -1
            temp[i+1][i] = -1
        temp[gridNum-1][gridNum-1] = 2
        temp[0][gridNum-1] = -1
        temp[gridNum-1][0] = -1
    ...
    def fdm1D(self, chargeDensity: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        potential = np.zeros(self.gridNum+1)
        electricField = np.zeros(self.gridNum+1)
        potential[0:-1] = ((self.invC).dot(chargeDensity.reshape((-1, 1)))).flatten()
        potential[-1] = potential[0]

        electricField[1:-1] = (potential[0:-2] - potential[2:]) / (2*self.gridLength)
        electricField[0] = ((potential[1] - potential[-2]) / 2*self.gridLength)
        electricField[-1] = electricField[0]
...
class PIC:
    ...
    """
    def __init__(self, T, L, A, CellN, W, Ne, Ni) -> None:
        # Spatial
        self.L = np.float64(L)      # World Length
        self.A = np.float64(A)      # World Area
        self.GN = int(CellN)    # Number of Grids
        self.grid_length = np.float64(L/(CellN))     # Grid Length
